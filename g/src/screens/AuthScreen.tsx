...
import { signInWithGoogle, signOutCurrentUser } from '@/services/authService'

export function AuthScreen() {
  const navigate = useNavigate()
  const { currentUser, isAdminMode, setCurrentUser } = useGame()
  ...
  async function handleSwitchAccount() {
    ...
    try {
      ...
      setMessage('לֹא הִצְלַחְנוּ לְנַתֵּק אֶת הַחֶשְׁבּוֹן כָּרֶגַע.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <ScreenLayout
      ...
      subtitle={
        isAdminMode
          ? 'מָסָךְ כְּנִיסָה פָּשׁוּט. כָּרֶגַע אֶפְשָׁר לְהַמְשִׁיךְ עִם פְּרוֹפִילֵי דֵּמוֹ, וּבַהֶמְשֵׁךְ חֶשְׁבּוֹן Google יְנַהֵל פְּרוֹפִילֵי יְלָדִים אֲמִתִּיִּים.'
          : 'מִתְחַבְּרִים עִם Google וּמַמְשִׁיכִים יָשָׁר לִיצִירַת יֶלֶד חָדָשׁ אוֹ לִבְחִירַת יֶלֶד קַיָּם.'
      }
    ...
    >
      <div className="space-y-6">
        ...
        <div className={`grid gap-6 ${isAdminMode ? 'lg:grid-cols-[1fr_0.95fr]' : ''}`}>
          <Card className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-600">
                ...
                אפשר לדמות כבר עכשיו את הזרימה המלאה של התחברות ויצירת ילד.
              </p>
            </div>
            {!currentUser ? (
              <>
              ...
              <>
                ...
                <Button disabled={isLoading} onClick={() => void handleSwitchAccount()} variant="secondary">
                  הַחֲלֵף לְחֶשְׁבּוֹן אַחֵר
                </Button>
              </>
            )}
